...
# Track already-processed approved files to avoid re-creating tasks
...
def process_approved_files():
    ...
    Detect the type from frontmatter and create the correct task:
    - linkedin_post_ready → APPROVED_LINKEDIN_* with type: linkedin_post
    - facebook_approval   → APPROVED_FACEBOOK_* with type: facebook_post
    - email_approval      → APPROVED_EMAIL_* with type: email_approval
    """
    if not APPROVED.exists():
        return

    for f in sorted(APPROVED.glob('*.md')):
        if f.name in processed_approved:
            continue
        try:
            content = f.read_text(encoding='utf-8')
            file_type = extract_frontmatter_field(content, 'type') or ''

            # --- LinkedIn Post ---
            if file_type == 'linkedin_post_ready' or 'LINKEDIN' in f.name.upper():
                topic = extract_frontmatter_field(content, 'topic') or 'general'
                logger.info(f"Approved LinkedIn post detected: {f.name}")
                log_audit('approved_linkedin', f.name)

                task_filename = f"APPROVED_LINKEDIN_{f.stem}.md"
...
"""
                ...
                logger.info(f"Created LinkedIn task: {task_filename}")

            # --- Facebook Post ---
            elif file_type == 'facebook_approval' or 'FACEBOOK' in f.name.upper():
                source_task = extract_frontmatter_field(content, 'source_task') or ''
                logger.info(f"Approved Facebook post detected: {f.name}")
                log_audit('approved_facebook', f.name)

                task_filename = f"APPROVED_FACEBOOK_{f.stem}.md"
                task_path = NEEDS_ACTION / task_filename
...
## Approved Facebook Post — Publish via MCP

This post has been approved by a human. Use the Facebook MCP tool `post_to_facebook` to publish it NOW.
...
"""
                task_path.write_text(task_content)
                logger.info(f"Created Facebook task: {task_filename}")
...
"""
                task_path.write_text(task_content)
                logger.info(f"Created email task: {task_filename}")

            # Mark as processed to avoid re-creating the task
